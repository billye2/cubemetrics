...
export async function addWordAction(word: string, definition: string, example: string) {
  const w = word.trim();
...
export async function deleteWordAction(id: number) {
  const { supabase, userId } = await requireUser();
  await supabase.from("vocab_words").delete().eq("id", id).eq("user_id", userId);
  revalidatePath(PATH);
}

/**
 * SM-2-lite scheduler — shared with flashcards. Maps a recall rating to the next
 * ease/interval/due date. "again" resets the word; better ratings push the next
 * review further out, scaled by the word's accumulated ease factor.
 */
