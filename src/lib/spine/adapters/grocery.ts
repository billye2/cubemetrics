import "server-only";
import { todayKey } from "@/lib/xp/tz";
import { checklistDueToday } from "../lib";
import type { SpineAdapter } from "../types";

// Read-only adapter: open grocery items (checklists list_type "grocery"). Surfaces "N to do"
// so a standing shopping list shows on Today; null when the list is empty/all checked.
export const adapter: SpineAdapter = {
  appId: "grocery",
  async today(ctx) {
    const today = todayKey(ctx.tz, ctx.now);
    const { data } = await ctx.supabase
      .from("checklists")
      .select("id, title, due_date")
      .eq("user_id", ctx.userId)
      .eq("list_type", "grocery")
      .eq("completed", false);
    const rows = (data ?? []) as { id: number; title: string; due_date: string | null }[];
    return checklistDueToday("grocery", rows, today);
  },
};
