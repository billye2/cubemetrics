...
- **P3** — **Photo** per recipe (Supabase Storage). **Link from Meal Planner**: let the meal planner reference a recipe so a planned meal opens its recipe. Optional per-step inline timers. _(Not started. `recipes.photo_path` column is reserved in the schema for the photo work.)_
  - [ ] Photo per recipe (Supabase Storage, owner-only bucket).
  - [ ] Link from Meal Planner so a planned meal opens its recipe.
  - [ ] Optional per-step inline timers.
